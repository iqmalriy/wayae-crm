...
async function detachCustomerFromOrgRequest(
  customerId: string,
): Promise<{ message: string; data: { customer: CustomerView } }> {
  const response = await fetch(
    `/api/v1/customers/${customerId}/detach-from-org`,
    { method: "POST" },
  );
  if (!response.ok) {
...
export type DetachCustomerFromOrgMutationOptions = UseMutationOptions<
  { message: string; data: { customer: CustomerView } },
  AppError,
  string
>;

export function useDetachCustomerFromOrganization(
  options?: DetachCustomerFromOrgMutationOptions,
) {
  const onSuccess = options?.onSuccess;
  const onError = options?.onError;
  return useMutation({
    mutationFn: (customerId: string) =>
      detachCustomerFromOrgRequest(customerId),
