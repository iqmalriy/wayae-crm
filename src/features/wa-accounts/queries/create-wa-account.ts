...
import type { CreateWaAccountOutput } from "../types/response-types";
...
interface SuccessBody {
  message: string;
  data: CreateWaAccountOutput;
}

async function createWaAccountRequest(
  input: CreateWaAccountInput,
): Promise<{ message: string; data: CreateWaAccountOutput }> {
  const response = await fetch("/api/v1/wa-accounts", {
    method: "POST",
...
export type CreateWaAccountMutationOptions = UseMutationOptions<
  { message: string; data: CreateWaAccountOutput },
  AppError,
  CreateWaAccountInput
>;

export function useCreateWaAccount(options?: CreateWaAccountMutationOptions) {
  const onSuccess = options?.onSuccess;
  const onError = options?.onError;
  return useMutation({
    mutationFn: createWaAccountRequest,
