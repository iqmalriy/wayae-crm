...
import { changeTaskStatusInput } from "../schemas/change-task-status.schema";
import { changeTaskStatusForUser } from "../services/change-task-status.service";
...
export async function changeTaskStatusHandler(
  ...
  const parsed = changeTaskStatusInput.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    ...
    const output = await changeTaskStatusForUser(
      id,
      {
    ...
    return NextResponse.json(ok({ task: output.task }), {
      status: 200,
    });
  } catch (error) {
